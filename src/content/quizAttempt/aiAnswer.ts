...
import { getDdmarkerChoices, getDdmarkerDropArea, setDdmarkerChoiceAnswer } from "./ddmarker";
import {
  getDdwtosChoices,
...
import {
  ...
  getDdimageOrTextDropGroupIndex,
...
export function getAiActionForSlot(
...
export function applyAiSelectAnswers(
  questionNode: Element,
  actions: AiAnswerAction[],
  ...
  selects.forEach((select, index) => {
    const slotCandidates = isMatchingQuestionNode(questionNode)
      ? [index + 1]
      : getSelectSlotIndexCandidates(select);
    const action =
      actions.find((candidate) =>
        slotCandidates.some((slotIndex) => candidate.slotIndex === slotIndex),
      ) ??
      actions[index] ??
      null;
    const directLabel = action ? getAiActionLabel(action) : "";
    const label =
      directLabel && findSelectOptionByLabel(select, directLabel)
        ? directLabel
...
export function applyAiEssayAnswer(
  questionNode: Element,
  actions: AiAnswerAction[],
  fallbackAnswer: string | null,
) {
...
export function applyAiOrderingAnswers(questionNode: Element, state: AiAnswerState) {
  ...
  const orderedLabels =
    positionedActions.length === itemCount
  ...
  return state.answer
    ? applyOrderingOrder(questionNode, splitSequentialAnswerLabels([state.answer], itemCount))
    : false;
}
    ...
    if (inputs.some((input) => input.type === "checkbox")) {
      ...
      for (const input of inputs) {
        ...
        changed =
          setAnswerInputChecked(
            input,
            [...labelKeys].some((key) => exactKeys.has(key)),
          ) || changed;
      }
    } else {
      const targetInput = inputs.find((input) =>
        labelsMatch(getInputAnswerLabelText(questionNode, input), label),
      );
      if (targetInput) {
...
export function applyAiAnswerForQuestion(questionNode: Element, state: AiAnswerState) {
  ...
  const actions =
    state.actions.length > 0 ? state.actions : state.answer ? [{ label: state.answer }] : [];
...
export function getAiQuestionKey(questionNode: Element | null, questionId: string | null) {
  ...
  return questionHash
    ? `hash:${questionHash}`
    : `text:${stableHashText(getQuestionText(questionNode))}`;
}
...
export function buildAiQuestionControls(questionNode: Element): AiQuestionControl[] {
  ...
  for (const [index, select] of getSelectableAnswerControls(questionNode).entries()) {
    controls.push({
      ...
      slotIndex: isMatchingQuestionNode(questionNode)
        ? index + 1
        : (getSelectPlaceIndex(select) ?? index + 1),
      index,
...
export async function buildAiAnswerRequestPayload(
  questionNode: Element,
  questionId: string | null,
) {
  return {
