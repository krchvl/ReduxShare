...
function calculatedSavePayload(
...
describe("ReduxShare review DB save (PocketBase)", () => {
  ...
  it("does not call PocketBase when Moodle courseId or quizId is missing", async () => {
    ...
    await expect(
      saveReduxShareReviewAnswers(authSession, calculatedSavePayload({ courseId: null })),
    ).resolves.toMatchObject({
      imported: false,
      savedCount: 0,
    });
    await expect(
      saveReduxShareReviewAnswers(authSession, calculatedSavePayload({ quizId: null })),
    ).resolves.toMatchObject({
      imported: false,
      savedCount: 0,
    });
  ...
  it("skips the PocketBase request entirely when no question has a usable id", async () => {
    ...
    expect(
      result.results.every((entry) => entry.ok && entry.data === null && entry.answerCount === 0),
    ).toBe(true);
  });
